...
use omj\financetools\fidavista\FidavistaAccount;
use omj\financetools\fidavista\FidavistaTransaction;
...
class FSMFidavistaStatement implements iStatement {
     ...
     */
    public $dbInterface;    //iAccountStatementDbController;
    //header data
    public $timestamp = ''; //eg, 20150823134808668 (2015-08-23 13:48 08.668)
    public $from = '';      //statement provider (bank info)
    //stmt dates
    public $startDate;
    public $endDate;
    public $prepDate;
        ...
        foreach ($this->accountList as $account) {
            ...
            foreach ($currencyList as $currency) {
                ...
                foreach ($trxList as $trx) {
                    ...
                    $arr = $this->getStatementRow($trx, $currencyName, $accountName);
                    $arr['currencyName'] = $currencyName;
                    $arr['openBal'] = $currency->openBalance();
                    $arr['closeBal'] = $currency->closeBalance();
                    $response[] = $arr;
                }
            }
        }
